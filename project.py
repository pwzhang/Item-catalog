    ...
    if request.method == 'POST':
        ...
        if username =='' or password=='' or email =='':
            response = make_response(
                json.dumps("Missing arguments."), 401)
            response.headers['Content-Type'] = 'application/json'
            return response
        if session.query(User).filter_by(name = username).first() is not None:
            response = make_response(
                json.dumps("User already exists."), 401)
            response.headers['Content-Type'] = 'application/json'
            return response
...
def restaurantMenuJSON(restaurant_id):
    if 'username' in login_session:
        ...
        flash("You have no authorization to get APIs.")
        return redirect(url_for('showRestaurants'))
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
        flash('you are not able to edit the restaurant')
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        flash('You are unable to delete the restaurant because you are not the owner.')
        return redirect(url_for('showMenu', restaurant_id = restaurant_id))
    if request.method == 'POST':
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if editedItem.user_id != login_session['user_id']:
        flash('You are unable to edit the item because you are not the owner.')
        return redirect(url_for('showMenu', restaurant_id = restaurant_id))
    if request.method == 'POST':
        if request.form['name']:
...
def deleteMenuItem(restaurant_id,menu_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        flash('You are unable to delete the item because you are not the owner.')
        return redirect(url_for('showMenu', restaurant_id = restaurant_id))
    if request.method == 'POST':
